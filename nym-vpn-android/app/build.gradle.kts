...
android {
	...
	defaultConfig {
		...
		buildConfigField("Boolean", "LAB_MOCK", "false")
		buildConfigField("String", "LAB_AUTH_BASE_URL", "\"\"")
		proguardFile("fdroid-rules.pro")

		ndk {
			abiFilters.clear()
			// Override with -Pabis=arm64-v8a (comma-separated) for smaller APKs.
			abiFilters.addAll(
				providers.gradleProperty("abis")
	...
	productFlavors {
		...
		create(Constants.LAB_MOCK) {
			dimension = Constants.TYPE
			applicationIdSuffix = ".labmock"
			buildConfigField("String", Constants.FLAVOR, "\"${Constants.LAB_MOCK}\"")
			buildConfigField("Boolean", "LAB_MOCK", "true")
			buildConfigField("String", "LAB_AUTH_BASE_URL", "\"http://104.250.122.199:8088/api/public/v1/lab/auth\"")
			resValue("string", "app_name", "NymVPN Lab")
		}
	}
...
val copyLabMockNetworkConfig = tasks.register<Copy>("copyLabMockNetworkConfig") {
	val labConfigDir = rootProject.projectDir.parentFile.resolve("local-run/config/nym/networks/mainnet")
	from(labConfigDir) {
		include("mainnet.json", "mainnet_discovery.json")
	}
	into(layout.projectDirectory.dir("src/labmock/assets/nym/networks/mainnet"))
}

tasks.named("preBuild") {
	dependsOn(copyLabMockNetworkConfig)
}
