...
Section Install
  ...
  ; Lab mock: wipe stale ProgramData then seed discovery + pinned gateways.
  ; Default entry/exit is Country CH; lab fleet is only US entry + DE exit →
  ; "Can't reach entry server" unless we pin identities (same as win-run-lab-vpnd.ps1).
  File "${RESPREFIX}\lab-mainnet.json"
  File "${RESPREFIX}\lab-mainnet_discovery.json"
  File "${RESPREFIX}\lab-nym-vpnd.json"
  ReadEnvStr $R9 PROGRAMDATA
  ${If} $R9 == ""
    StrCpy $R9 "C:\ProgramData"
  ${EndIf}
  ; Stop service if already installed so files under ProgramData can be removed
  nsExec::ExecToLog 'net stop nym-vpnd'
  Pop $0
  DetailPrint "net stop nym-vpnd: $0"
  RmDir /r "$R9\nym-vpnd"
  CreateDirectory "$R9\nym-vpnd\config\networks\mainnet"
  CopyFiles /SILENT "$INSTDIR\lab-mainnet.json" "$R9\nym-vpnd\config\networks\mainnet\mainnet.json"
  CopyFiles /SILENT "$INSTDIR\lab-mainnet_discovery.json" "$R9\nym-vpnd\config\networks\mainnet\mainnet_discovery.json"
  CopyFiles /SILENT "$INSTDIR\lab-nym-vpnd.json" "$R9\nym-vpnd\config\nym-vpnd.json"
