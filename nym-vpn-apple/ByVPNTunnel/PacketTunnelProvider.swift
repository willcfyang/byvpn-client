...
class PacketTunnelProvider: NEPacketTunnelProvider {
    ...
    var vpnService: ByVpnService?
    var commandSender: ByVpnServiceCommandSender?
    ...
    override init() {
        ...
        LoggingSystem.bootstrap { label in
            ...
            let osLogHandler = OSLogHandler(
                subsystem: Bundle.main.bundleIdentifier ?? "ByVPNTunnel",
                category: label
            )
...
extension PacketTunnelProvider {
    func setup(vpnConfig: VpnConfig) async throws {
        ...
        vpnService = try await ByVpnService.newService(
            config: vpnConfig,
