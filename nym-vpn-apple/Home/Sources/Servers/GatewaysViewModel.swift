...
@MainActor public class GatewaysViewModel: ObservableObject {
    ...
    @Published var countries = [ByVpnCountry]()
    @Published var foundCountries = [ByVpnCountry]()
    @Published var foundRegions = [(country: ByVpnCountry, region: String)]()
    ...
    func searchCountriesGateways() {
        Task { [weak self] in
            guard let self, searchText.count >= minimumSearchSymbols
                ...
                await MainActor.run {
                    self?.foundCountries = [ByVpnCountry]()
            ...
            var seen = Set<String>()
            let newCountryRegionPairs: [(country: ByVpnCountry, region: String)] = gateways
                .compactMap { gateway -> (ByVpnCountry, String)? in
