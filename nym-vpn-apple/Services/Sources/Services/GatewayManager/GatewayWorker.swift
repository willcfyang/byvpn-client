...
actor GatewayWorker {
    ...
    func countries(from nodes: [GatewayNode]) -> [ByVpnCountry] {
        ...
        var result: [ByVpnCountry] = []
        result.reserveCapacity(citiesByRegionByCountry.count)

        for (code, regionsDict) in citiesByRegionByCountry {
            let regions: [ByVpnCountry.Region] = regionsDict
                .map { regionName, citiesSet in
                    let cities = citiesSet.sorted {
                        $0.localizedCaseInsensitiveCompare($1) == .orderedAscending
                    }
                    return ByVpnCountry.Region(name: regionName, cities: cities)
                }
                .sorted { $0.name.localizedCaseInsensitiveCompare($1.name) == .orderedAscending }

            result.append(ByVpnCountry(name: code, code: code, regions: regions)) // name localized later
        }
