...
private extension MixnetTuningView {
    ...
    var subtitle: some View {
        HStack(spacing: 0) {
            Text("mixnetTunning.subtitle".localizedString)
                .byVpnText(color: Color.ByVpn.textSecondary, style: .Body.Medium.regular)
            Spacer()
        }
    }
    ...
    func separatorLine() -> some View {
        Rectangle()
            .foregroundColor(Color.ByVpn.gray2)
            .frame(height: 1)
            .padding(.vertical, 12)
    }
    ...
    @ViewBuilder var backgroundCoverTrafficSection: some View {
        Text("⚠️ \("mixnetTuning.sendTraffic.off".localizedString)")
            .byVpnText(color: Color.ByVpn.warning, style: .Body.Small.regular)
        Spacer()
            .frame(height: 16)
        Text("mixnetTuning.backgroundCoverTrafficState.title".localizedString)
            .byVpnText(color: Color.ByVpn.textPrimary, style: .Headline.Small.regular)
        Spacer()
            .frame(height: 16)
        Text("mixnetTuning.backgroundCoverTrafficState.subtitle".localizedString)
            .byVpnText(color: Color.ByVpn.textSecondary, style: .Body.Small.regular)
        Spacer()
            .frame(height: 16)
    ...
    @ViewBuilder var coverTrafficSliderSection: some View {
        ...
        HStack(spacing: 0) {
            Text("\("mixnetTuning.base".localizedString)\n")
                .byVpnText(color: Color.ByVpn.textPrimary, style: .Body.Medium.regular)
        ...
        return HStack(spacing: 0) {
            ...
            if mixingDelayIndex != Double(defaultDelay.defaultValue) {
                Text("\("mixnetTuning.current".localizedString)\n \(Int(mixingDelayIndex)) ms")
                    .byVpnText(color: Color.ByVpn.info, style: .Body.Medium.regular)
                    .multilineTextAlignment(.center)
            } else {
    ...
    var learnMoreAttributtedString: AttributedString {
        ...
        first.foregroundColor = Color.ByVpn.textPrimary
        first.link = URL(string: Constants.mixnetParametersLearnMoreURL.rawValue)
        return first
    }

    var learnMoreLink: some View {
        ExternalLink(text: learnMoreAttributtedString, color: Color.ByVpn.textPrimary, style: .Body.Small.regular)
            .onTapGesture {
