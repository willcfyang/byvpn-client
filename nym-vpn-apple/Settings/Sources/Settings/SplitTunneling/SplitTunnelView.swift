...
private extension SplitTunnelView {
    ...
    func fullDiskAccessAttributtedString() -> AttributedString {
        ...
        second.foregroundColor = Color.ByVpn.primary
            ...
            HStack(spacing: 0) {
                ZStack {
                    if isEnabled {
                        ...
                        Color.ByVpn.error.opacity(0.10)
                    }
                    Image(systemName: "slash.circle.fill")
                        .font(.system(size: 12, weight: .semibold))
                        .foregroundStyle(isEnabled ? Color.ByVpn.textSecondary : Color.ByVpn.error)
                }
                .frame(maxWidth: .infinity, maxHeight: .infinity)

                Rectangle()
                    .fill(Color.ByVpn.gray2)
                    .frame(width: 1)

                ZStack {
                    if isEnabled {
                        Color.ByVpn.primary.opacity(0.10)
                    } else {
                        Color.clear
                    }
                    Image(systemName: "shield.fill")
                        .font(.system(size: 12, weight: .semibold))
                        .foregroundStyle(isEnabled ? Color.ByVpn.primary : Color.ByVpn.textSecondary)
                }
                .frame(maxWidth: .infinity, maxHeight: .infinity)
            }
...
private struct SectionIndexOverlay: View {
    ...
    private func letterColor(for letter: String) -> Color {
        if letter == draggedLetter {
            return Color.ByVpn.textPrimary
        }
        return sections.contains(where: { $0.title == letter })
            ? Color.ByVpn.textSecondary
            : Color.ByVpn.gray2
    }
