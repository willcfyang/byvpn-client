...
private extension ActionDialogView {
    ...
    func icon() -> some View {
        if let iconImageName = viewModel.configuration.systemIconImageName {
            ...
            Image(systemName: iconImageName)
                .foregroundStyle(viewModel.configuration.systemIconImageColor ?? Color.ByVpn.textPrimary)
    ...
    func title() -> some View {
        if let title = viewModel.configuration.titleLocalizedString {
            Text(title)
                .textStyle(ByVpnTextStyle.Headline.Medium.regular)
    ...
    func noButton(text: String) -> some View {
        GenericButton(
            title: text,
            titleColor: viewModel.configuration.isNoDestructive ? Color.ByVpn.error : nil,
            style: .textOnly
        )
        .onTapGesture {
#if os(iOS)
    ...
    func loadingRow() -> some View {
        if let loadingText = viewModel.configuration.loadingText {
            HStack(alignment: .center) {
                Text(loadingText)
                    .textStyle(ByVpnTextStyle.Body.Large.regular)
                    .foregroundStyle(Color.ByVpn.textPrimary)
                Spacer()
                    .frame(width: 8)
