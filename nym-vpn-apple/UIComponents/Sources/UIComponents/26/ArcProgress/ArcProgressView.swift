...
private extension ArcProgressView {
    ...
    var sphereFill: AnyShapeStyle {
        if colorScheme == .dark {
            AnyShapeStyle(
                RadialGradient(
                    colors: [
                        Color.ByVpn.sphereGradientTop,
                        Color.ByVpn.sphereGradientBottom
                    ],
        ...
        if state == .connected, let connectedDate {
            TimelineView(.periodic(from: connectedDate, by: 1.0)) { context in
                Text(verbatim: fastHMS(from: connectedDate, to: context.date))
                    .font(.system(size: Constants.labelFontSize))
                    .foregroundColor(Color.ByVpn.secondary)
                    .monospacedDigit()
            }
            .id(connectedDate)
    ...
    var haloColor: Color {
        ...
        switch mode {
        case .fast:
            return Color.ByVpn.secondary.opacity(0.55)
        case .anonymous:
            return Color.ByVpn.gray1.opacity(0.35)
        }
    }
    ...
    var labelColor: Color {
        ...
        case .connected, .step:     return Color.ByVpn.secondary
        case .disconnected, .canceling: return Color.ByVpn.textTertiary
        }
    }
    ...
    enum Constants {
        ...
        static let fastFill      = Color.ByVpn.secondary
        static let anonymousFill = Color.ByVpn.anonymousArc.opacity(0.60)
        static let track         = Color.white.opacity(0.15)
        static let errorFill     = Color.ByVpn.error.opacity(0.60)
        static let errorTint     = Color.ByVpn.error.opacity(0.08)
    }
}
