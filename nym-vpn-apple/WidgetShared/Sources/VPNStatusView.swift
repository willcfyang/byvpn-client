...
private extension VPNStatusView {
    ...
    var iconColor: Color {
        switch entry.status {
        case .error:
            return ByVpnWidgetColors.error
        case .status(let neStatus):
            switch neStatus {
            case .connected, .reasserting:
                return ByVpnWidgetColors.accent
            default:
                return .secondary
            }
        case .notConfigured:
    ...
    var statusText: some View {
        Group {
            if entry.status.isConnecting {
                ...
                Text("Connected")
                    .foregroundStyle(ByVpnWidgetColors.accent)
            } else {
                switch entry.status {
                case .error:
                    Text("Error")
                        .foregroundStyle(ByVpnWidgetColors.error)
                default:
                    Text("Disconnected")
                }
            }
