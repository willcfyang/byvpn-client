...
/// Lab: keep physical uplink usable (Cursor, DNS); only `NYM_VPN_LAB_ROUTE_CIDRS` use the tunnel.
fn lab_physical_default() -> bool {
    env::var("NYM_VPN_LAB_PHYSICAL_DEFAULT")
        .ok()
...
impl<'a> PolicyBatch<'a> {
    ...
    fn add_policy_specific_rules(&mut self, policy: &FirewallPolicy, fwmark: u32) -> Result<()> {
        ...
        if lab_physical_default() {
            tracing::info!(
                "NYM_VPN_LAB_PHYSICAL_DEFAULT: skip kill-switch reject (physical uplink allowed)"
            );
            return Ok(());
        }
