...
use nym_credentials_interface::{
    ...
    WithdrawalRequest, ecash_parameters,
};
...
impl RequestZkNymTask {
    ...
    async fn import_aggregated_coin_index_signatures(
        ...
        let expected_count = ecash_parameters().get_total_coins() as usize;
        let incoming_count = aggregated_coin_index_signatures.signatures.signatures.len();
        if incoming_count != expected_count {
            tracing::error!(
                "Coin index signature count from API ({incoming_count}) does not match ticketbook size ({expected_count})"
            );
            return Err(RequestZkNymError::ImportZkNym {
                ticket_type: "coin-index-signatures".to_string(),
                error: format!(
                    "expected {expected_count} coin index signatures, got {incoming_count}"
                ),
            });
        }
        ...
        let should_store = match stored.as_ref().map(|s| s.len()) {
            None => true,
            Some(len) if len != incoming_count => {
                tracing::warn!(
                    "Replacing stale coin index signatures for epoch {epoch_id} ({len} -> {incoming_count})"
                );
                true
            }
            Some(_) => false,
