...
impl NymVpnService {
    ...
    pub async fn new(
        ...
        // Use discovery-preferred VPN API URLs (see Network::nym_vpn_api_urls). from_network()
        // reads NymNetworkDetails only, which can still be production while discovery is lab.
        let vpn_api_urls = network_env
            .nym_vpn_api_urls()
            .ok_or(Error::InvalidEnvironment("empty nym_vpn_api_urls"))?;
        let vpn_api_urls = api_urls_to_urls(&vpn_api_urls).map_err(Error::ConvertApiUrls)?;
        tracing::info!(
            "VpnApiClient base URL(s): {:?}",
            vpn_api_urls
                .iter()
