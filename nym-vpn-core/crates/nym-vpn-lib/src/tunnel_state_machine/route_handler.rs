...
impl RouteHandler {
    ...
    pub async fn add_routes(
        ...
        #[cfg(target_os = "linux")]
        if !Self::lab_physical_default() {
            self.route_manager.create_routing_rules(enable_ipv6).await?;
        } else {
            tracing::info!(
                "NYM_VPN_LAB_PHYSICAL_DEFAULT: skipping full-tunnel policy rules; \
                 only NYM_VPN_LAB_ROUTE_CIDRS use the tunnel"
            );
        }
    ...
    fn get_routes(routing_config: RoutingConfig, enable_ipv6: bool) -> HashSet<RequiredRoute> {
        ...
        match routing_config {
            ...
            RoutingConfig::Wireguard {
                ...
                routes.extend(Self::lab_or_default_routes(
                    exit_tun_name,
                    exit_tun_mtu,
                    enable_ipv6,
                ));
    ...
    #[cfg(target_os = "linux")]
    fn lab_physical_default() -> bool {
        std::env::var("NYM_VPN_LAB_PHYSICAL_DEFAULT")
            .ok()
            .is_some_and(|v| matches!(v.as_str(), "1" | "true" | "yes" | "on"))
    }

    /// Full tunnel, or lab split: only `NYM_VPN_LAB_ROUTE_CIDRS` via exit tun (main table).
    /// Exit gateway public IP is routed via entry tun (multihop); see `get_multihop_exit_route`.
    fn lab_or_default_routes(
        iface: String,
        mtu: u16,
        enable_ipv6: bool,
    ) -> Vec<RequiredRoute> {
        #[cfg(target_os = "linux")]
        if Self::lab_physical_default() {
            return Self::get_lab_split_routes(iface, mtu);
        }
        Self::get_default_routes(iface, mtu, enable_ipv6)
    }

    #[cfg(target_os = "linux")]
    fn get_lab_split_routes(iface: String, mtu: u16) -> Vec<RequiredRoute> {
        let cidrs = std::env::var("NYM_VPN_LAB_ROUTE_CIDRS").unwrap_or_else(|_| {
            "10.1.0.0/16".to_string()
        });
        let mut routes = Vec::new();
        for cidr in cidrs.split_whitespace() {
            let Ok(prefix) = cidr.parse::<IpNetwork>() else {
                tracing::warn!("NYM_VPN_LAB_ROUTE_CIDRS: ignoring invalid CIDR {cidr:?}");
                continue;
            };
            let mut route = RequiredRoute::new(prefix, Node::device(iface.clone()));
            route = route.use_main_table(true).mtu(mtu);
            routes.push(route);
            tracing::info!("NYM_VPN_LAB: route {cidr} via {iface} (main table)");
        }
        routes
    }
