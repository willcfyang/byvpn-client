...
/// When set (`1` / `true` / `yes`), skip ICMP/TCP probe to `1.1.1.1` and emit `Viable` immediately.
/// For colocated mock labs where exit NAT return breaks the probe but WG registration succeeded.
fn lab_skip_connection_probe() -> bool {
    matches!(
        std::env::var("NYM_VPN_LAB_SKIP_CONNECTION_PROBE")
            .ok()
            .map(|v| v.to_ascii_lowercase())
            .as_deref(),
        Some(s) if matches!(s, "1" | "true" | "yes")
    )
}

/// Lab split routing: probe a target reachable via `NYM_VPN_LAB_ROUTE_CIDRS` (e.g. colocated exit).
fn lab_probe_ipv4() -> Option<Ipv4Addr> {
    std::env::var("NYM_VPN_LAB_PROBE_IP")
        .ok()
        .and_then(|v| v.parse().ok())
}

/// Timeout for starting the registration client
const REGISTRATION_CLIENT_STARTUP_TIMEOUT: Duration = Duration::from_secs(15);
...
impl TunnelMonitor {
    ...
    async fn run_inner(&mut self) -> Result<Tombstone> {
        ...
        tracing::info!(
            ...
            step = "S4_nodes_resolved",
            entry = %selected_gateways.entry_gateway().identity(),
            exit = %selected_gateways.exit_gateway().identity(),
        );
        ...
        tracing::info!(
            event = "registration.step",
            step = "S5_registration_nym_nodes_built",
            entry_auth = ?entry_node.node.authenticator_address,
            exit_auth = ?exit_node.node.authenticator_address,
            entry_ip = ?entry_node.node.ip_address,
            exit_ip = ?exit_node.node.ip_address,
        );
        ...
        tracing::info!(
            event = "registration.step",
            step = "S6_registration_client_build_start",
            mode = ?mode,
            mixnet_startup_timeout_secs = REGISTRATION_CLIENT_STARTUP_TIMEOUT.as_secs(),
        );
        let registration_client = Box::pin(rc_builder.build()).await.map_err(|e| {
            tracing::error!(
                event = "registration.step",
                step = "S6_registration_client_build_failed",
                error = %e,
            );
            e
        })?;
        tracing::info!(
            event = "registration.step",
            step = "S7_registration_client_build_ok",
        );
        ...
        let tunnel_connection_monitor_handle = if lab_skip_connection_probe() {
            tracing::warn!(
                event = "lab.skip_connection_probe",
                "NYM_VPN_LAB_SKIP_CONNECTION_PROBE: skipping ICMP/TCP viability probe; declaring tunnel viable immediately"
            );
            let now = tokio::time::Instant::now();
            let _ = tunnel_connection_monitor_tx.send(ConnectionEvent {
                start_timestamp: now,
                end_timestamp: now,
            ...
            self.create_tunnel_connection_monitor(
    ...
    async fn start_wireguard_netstack_tunnel(
        ...
        let exit_endpoint = conn_data.exit.endpoint.ip();
        let dns_servers = self.get_mobile_dns_addresses();

        let mut remote_addresses = vec![entry_endpoint];
        if exit_endpoint != entry_endpoint {
            remote_addresses.push(exit_endpoint);
        }
    ...
    fn create_icmp_probe(&self, exit_tunnel_metadata: &TunnelMetadata) -> Result<IcmpProbe> {
        let mut icmp_probe_config = if let Some(probe_ip) = lab_probe_ipv4() {
            tracing::info!(
                event = "lab.connection_probe_ip",
                %probe_ip,
                "NYM_VPN_LAB_PROBE_IP: ICMP viability probe via exit tunnel"
            );
            IcmpProbeConfig::new(probe_ip)
        } else {
            IcmpProbeConfig::default_v4()
        };
    ...
    fn create_tcp_probe(&self, exit_tunnel_metadata: &TunnelMetadata) -> Result<TcpProbe> {
        let mut tcp_probe_config = if let Some(probe_ip) = lab_probe_ipv4() {
            TcpProbeConfig::new(SocketAddr::new(IpAddr::V4(probe_ip), 80))
        } else {
            TcpProbeConfig::default_v4()
        };
