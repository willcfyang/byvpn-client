...
impl Network {
    ...
    pub fn nym_vpn_api_urls(&self) -> Option<Vec<nym_network_defaults::ApiUrl>> {
        // Prefer discovery-sourced URLs (NymVpnNetwork). Bundled NymNetworkDetails::new_mainnet()
        // still points at production; after lab discovery load those can disagree until patched.
        if !self.nym_vpn_network.nym_vpn_api_urls.is_empty() {
            Some(self.nym_vpn_network.nym_vpn_api_urls.clone())
        } else {
...
impl NetworkCache {
    ...
    //
    // VpnApiClient is built from NymNetworkDetails. Bundled mainnet defaults still point at
    // production (nymvpn.com); lab seed / embedded discovery may point at the mock API.
    // Always prefer discovery API URLs so account sync does not hit production and report
    // "Account not found" / inactive after lab login.
    fn patch_network_details_from_discovery(
        network_details: &mut NymNetworkDetails,
        discovery: &Discovery,
    ) {
        let discovery_vpn_urls = discovery.nym_vpn_api_urls();
        if !discovery_vpn_urls.is_empty() {
            let needs_patch = network_details.nym_vpn_api_urls.as_ref() != Some(&discovery_vpn_urls);
            if needs_patch {
                tracing::info!(
                    "Applying nym_vpn_api_urls from discovery ({:?})",
                    discovery_vpn_urls
                        .first()
                        .map(|u| u.url.as_str())
                        .unwrap_or("")
                );
                network_details.nym_vpn_api_urls = Some(discovery_vpn_urls);
            }
        }
        let discovery_api_urls = discovery.nym_api_urls();
        if !discovery_api_urls.is_empty() {
            let needs_patch = network_details.nym_api_urls.as_ref() != Some(&discovery_api_urls);
            if needs_patch {
                tracing::info!(
                    "Applying nym_api_urls from discovery ({:?})",
                    discovery_api_urls
                        .first()
                        .map(|u| u.url.as_str())
                        .unwrap_or("")
                );
                network_details.nym_api_urls = Some(discovery_api_urls);
            }
        }
        if let Some(url) = discovery.nym_vpn_api_urls().first() {
            network_details.nym_vpn_api_url = Some(url.url.clone());
        }
    }
