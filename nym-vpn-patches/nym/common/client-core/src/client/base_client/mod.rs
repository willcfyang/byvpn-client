    ...
    C: DkgQueryClient + Send + Sync + 'static,
{
        ...
        <S::GatewaysDetailsStore as GatewaysDetailsStore>::StorageError: Sync + Send,
    {
        ...
        let ecash_ticket_type = match config.debug.remember_me.session_type() {
            nym_statistics_common::types::SessionType::Vpn => {
                nym_credentials_interface::TicketType::V1WireguardEntry
            }
            _ => nym_credentials_interface::TicketType::V1MixnetEntry,
        };
        let session_gateway_cfg = GatewayClientConfig::new_default()
            .with_disabled_credentials_mode(config.client.disabled_credentials_mode)
            .with_ecash_ticket_type(ecash_ticket_type)
        ...
        let mut gateway_client =
            if let Some(existing_client) = initialisation_result.authenticated_ephemeral_client {
                let mut gateway_client = existing_client.upgrade(
                    packet_router,
                    bandwidth_controller,
                    stats_reporter,
                    shutdown_tracker.clone_shutdown_token(),
                );
                // Ephemeral registration handshake uses disabled credentials; restore session cfg.
                gateway_client.cfg.bandwidth = session_gateway_cfg.bandwidth;
                gateway_client.cfg.connection.response_timeout_duration =
                    session_gateway_cfg.connection.response_timeout_duration;
                gateway_client
            } else {
