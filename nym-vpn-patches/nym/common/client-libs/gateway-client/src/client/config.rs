...
use nym_credentials_interface::{DEFAULT_MIXNET_REQUEST_BANDWIDTH_THRESHOLD, TicketType};
...
impl GatewayClientConfig {
    ...
    #[must_use]
    pub fn with_ecash_ticket_type(mut self, ticket_type: TicketType) -> Self {
        self.bandwidth.ecash_ticket_type = Some(ticket_type);
        self
    }
}
...
pub struct BandwidthTickets {
    ...
    /// Ecash ticketbook type for gateway bandwidth claims. Defaults to [`TicketType::V1MixnetEntry`].
    pub ecash_ticket_type: Option<TicketType>,
}
