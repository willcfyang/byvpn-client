...
impl CredentialVerifier {
    ...
    pub async fn verify(&mut self) -> Result<i64> {
        if self.ecash_verifier.is_mock() {
            // Mock mode: skip crypto verification but still credit server-side bandwidth.
            // Previously we only returned MOCK_BANDWIDTH in the WS response while leaving
            // BandwidthStorageManager at 0B, so handle_forward_sphinx rejected all sphinx.
            let bandwidth = Bandwidth::new_unchecked(MOCK_BANDWIDTH as u64);
