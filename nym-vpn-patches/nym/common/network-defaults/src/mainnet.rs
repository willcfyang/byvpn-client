...
// Lab / vpn.sf mock API (matches onidel-patches discovery). Production nymvpn.com
// must not be used here: VpnApiClient is built from these defaults when cache is empty,
// and a production sync returns "Account not found" → account inactive after lab login.
pub const NYM_API: &str = "http://104.250.122.199:8088/api/";
#[cfg(feature = "network")]
pub const NYM_APIS: &[ApiUrlConst] = &[ApiUrlConst {
    url: NYM_API,
    front_hosts: None,
}];

pub const NYM_VPN_API: &str = "http://104.250.122.199:8088/api/";
...
#[cfg(feature = "network")]
pub const NYM_VPN_APIS: &[ApiUrlConst] = &[ApiUrlConst {
    url: NYM_VPN_API,
    front_hosts: None,
}];
