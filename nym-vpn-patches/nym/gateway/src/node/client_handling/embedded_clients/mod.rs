...
impl MessageRouter {
    ...
    fn handle_received_messages(&self, messages: Vec<Vec<u8>>) {
        ...
        for packet in &messages {
            if packet.len() == PacketSize::AckPacket.plaintext_size() {
                acks += 1;
            } else if packet.len()
                == PacketSize::RegularPacket.plaintext_size() - sphinx_ack_overhead
            {
                data += 1;
            } else {
                data += 1;
        ...
        if acks > 0 || data > 0 {
            tracing::info!(
                ...
                step = "S9_embedded_route_batch",
                acks,
                data,
                batch_len = messages.len(),
            );
        }
