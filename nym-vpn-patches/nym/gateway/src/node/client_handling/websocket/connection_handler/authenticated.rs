...
impl<R, S> AuthenticatedHandler<R, S> {
    ...
    fn forward_packet(&self, mix_packet: MixPacket) {
        tracing::info!(
            ...
            step = "S9_gw_ws_forward_mix",
            client = %self.client.address,
            packet_type = ?mix_packet.packet_type(),
        );
        if let Err(err) = self
            .inner
            .shared_state
    ...
    async fn handle_forward_sphinx(
        ...
        let remaining_bandwidth = if self.upgrade_mode_enabled() {
                ...
                .await
            {
                ...
                Err(nym_credential_verification::Error::OutOfBandwidth {
                    required,
                    available,
                }) => {
                    tracing::warn!(
                        event = "registration.step",
                        step = "S9_gw_ws_forward_rejected",
                        client = %self.client.address,
                        required,
                        available,
                    );
                    return Err(RequestHandlingError::from(
                        nym_credential_verification::Error::OutOfBandwidth {
                            required,
                            available,
                        },
