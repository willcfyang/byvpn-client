...
pub async fn create_mixnet_client(
    ...
    // vpn.sf lab: embedded auth SP was flooding ack-only pushes to its own identity,
    // starving real Init handling on mixnet_client.next().
    ...
    tracing::info!(
        ...
        step = "S9_auth_sp_mixnet_debug_config",
        disable_loop_cover = debug_config.cover_traffic.disable_loop_cover_traffic_stream,
        disable_mix_hops = debug_config.traffic.disable_mix_hops,
    );
