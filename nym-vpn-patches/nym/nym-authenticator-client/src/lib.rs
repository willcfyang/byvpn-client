...
impl AuthenticatorClient {
    ...
    async fn send_request(&self, message: &ClientMessage) -> Result<u64> {
        ...
        tracing::info!(
            ...
            step = "S8_auth_init_sent",
            request_id,
            payload_len,
            auth_gateway = %self.auth_recipient.gateway(),
            uses_surbs = message.use_surbs(),
        );
    ...
    fn handle_response(
        ...
        if response.id() == request_id {
            tracing::info!(
                event = "registration.step",
                step = "S8_auth_listen_matched",
                request_id,
                gateway_ip = %self.ip_addr,
            );
    ...
    async fn listen_for_response(&mut self, request_id: u64) -> Result<AuthenticatorResponse> {
        ...
        tracing::info!(
            event = "registration.step",
            step = "S8_auth_listen_start",
            request_id,
            gateway_ip = %self.ip_addr,
            auth_gateway = %self.auth_recipient.gateway(),
            timeout_secs = AUTH_RESPONSE_TIMEOUT.as_secs(),
        );
        let timeout = tokio::time::sleep(AUTH_RESPONSE_TIMEOUT);
        tokio::pin!(timeout);
        ...
        loop {
            tokio::select! {
                _ = &mut timeout => {
                    error!(
                        event = "registration.step",
                        step = "S8_auth_listen_timeout",
                        request_id,
                        gateway_ip = %self.ip_addr,
                        "Timed out waiting for reply to connect request"
                    );
                ...
                msg = self.mixnet_listener.recv() => match msg {
                    ...
                    Ok(msg) => {
                        tracing::debug!(
                            event = "registration.step",
                            step = "S8_auth_listen_got_mixnet_msg",
                            request_id,
    ...
    pub async fn register_wireguard(
        ...
        tracing::info!(
            event = "registration.step",
            step = "S8_auth_register_wireguard_start",
            gateway_ip = %self.ip_addr,
            auth_gateway = %self.auth_recipient.gateway(),
            ?ticketbook_type,
        );
