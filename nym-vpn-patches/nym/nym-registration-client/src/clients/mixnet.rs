...
impl MixnetBasedRegistrationClient {
    ...
    async fn register_wg(self) -> Result<RegistrationResult, RegistrationError> {
        ...
        // Serial entry then exit (lab): avoids parallel Init on one AuthClientMixnetListener.
        tracing::info!(
            event = "registration.step",
            step = "S8a_entry_register_wireguard_start",
        );
        let entry = match self
            .cancel_token
            .run_until_cancelled(entry_auth_client.register_wireguard(
        ...
        tracing::info!(
            event = "registration.step",
            step = "S8a_entry_register_wireguard_done",
            ok = entry.is_ok(),
        );

        tracing::info!(
            event = "registration.step",
            step = "S8b_exit_register_wireguard_start",
        );
        let exit = match self
            .cancel_token
            .run_until_cancelled(exit_auth_client.register_wireguard(
                &*self.bandwidth_controller,
        ...
        tracing::info!(
            event = "registration.step",
            step = "S8b_exit_register_wireguard_done",
            ok = exit.is_ok(),
        );
